 ...
 *
 * Shown to the devices that can let people into a group — the ones who set
 * it up — and to a device in no group yet; the other members have nothing to
 * do about it, and a technical notice would only worry them.
 */
function runsTheDatabase() {
  const held = groups();
  return !held.length || held.some((group) => group.admits);
}
